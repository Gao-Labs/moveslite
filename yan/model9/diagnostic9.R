#' @title diagnostic workflow # 9
#' @author Yan
#' @describeIn diagnostic workflow with model 9


# model = lm(emissions ~ sqrt(vmt) + sqrt(vehicles) + sqrt(sourcehours) +
#         sqrt(starts) + year, data = default)
...
for (i in 1:3110) {
  ...
  table_list[[i]] <- diagnostic(.data = default, .formula = emissions ~ sqrt(vmt) + sqrt(vehicles) + sqrt(sourcehours) + sqrt(starts) + year, .pollutant = .pollutant, .by = .by, .geoid = .table)

}

by_16_9 <- do.call(rbind, table_list)  # Using rbind()

by_16_9 %>%
  write_csv("yan/model9/table_by_16_9.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:13) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~ sqrt(vmt) + sqrt(vehicles) + sqrt(sourcehours) + sqrt(starts) + year, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = sourcetype_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_8_9 <- do.call(rbind, table_list)  # Using rbind()

by_8_9 %>%
  write_csv("yan/model9/table_by_8_9.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:6) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~ sqrt(vmt) + sqrt(vehicles) + sqrt(sourcehours) + sqrt(starts) + year, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = fueltype_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_14_9 <- do.call(rbind, table_list)  # Using rbind()
by_14_9 %>%
  write_csv("yan/model9/table_by_14_9.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:5) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~ sqrt(vmt) + sqrt(vehicles) + sqrt(sourcehours) + sqrt(starts) + year, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = roadtype_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_15_9 <- do.call(rbind, table_list)  # Using rbind()

by_15_9 %>%
  write_csv("yan/model9/table_by_15_9.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:9) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~ sqrt(vmt) + sqrt(vehicles) + sqrt(sourcehours) + sqrt(starts) + year, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = regclass_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_12_9 <- do.call(rbind, table_list)  # Using rbind()

by_12_9 %>%
  write_csv("yan/model9/table_by_12_9.csv")
