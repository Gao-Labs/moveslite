#' @title diagnostic workflow # 3
#' @author Yan
#' @describeIn diagnostic workflow with model 3


# model = lm(emissions ~ poly(vmt, 2) + vehicles + poly(sourcehours, 2) +
#          starts + year, data = default)
...
for (i in 1:3110) {
  ...
  table_list[[i]] <- diagnostic(.data = default, .formula = emissions ~ poly(vmt, 2) + vehicles + poly(sourcehours, 2) + starts + year, .pollutant = .pollutant, .by = .by, .geoid = .table)

}

by_16_3 <- do.call(rbind, table_list)  # Using rbind()

by_16_3 %>%
  write_csv("yan/model3/table_by_16_3.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:13) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~ poly(vmt, 2) + vehicles + poly(sourcehours, 2) + starts + year, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = sourcetype_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_8_3 <- do.call(rbind, table_list)  # Using rbind()

by_8_3 %>%
  write_csv("yan/model3/table_by_8_3.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:6) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~ poly(vmt, 2) + vehicles + poly(sourcehours, 2) + starts + year, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = fueltype_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_14_3 <- do.call(rbind, table_list)  # Using rbind()
by_14_3 %>%
  write_csv("yan/model3/table_by_14_3.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:5) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~ poly(vmt, 2) + vehicles + poly(sourcehours, 2) + starts + year, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = roadtype_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_15_3 <- do.call(rbind, table_list)  # Using rbind()

by_15_3 %>%
  write_csv("yan/model3/table_by_15_3.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:9) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~ poly(vmt, 2) + vehicles + poly(sourcehours, 2) + starts + year, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = regclass_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_12_3 <- do.call(rbind, table_list)  # Using rbind()

by_12_3 %>%
  write_csv("yan/model3/table_by_12_3.csv")
