#' @title diagnostic workflow # 2
#' @author Yan
#' @describeIn diagnostic workflow with model 2
...
for (i in 1:3110) {
  ...
  table_list[[i]] <- diagnostic(.data = default, .formula = emissions ~  year + vmt + sourcehours + vehicles + starts +
                                  year*vmt + year*sourcehours + vmt*sourcehours, .pollutant = .pollutant, .by = .by, .geoid = .table)

}

by_16_2 <- do.call(rbind, table_list)  # Using rbind()

by_16_2 %>%
  write_csv("yan/model2/table_by_16_2.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:13) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~  year + vmt + sourcehours + vehicles + starts +
                           year*vmt + year*sourcehours + vmt*sourcehours, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = sourcetype_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_8_2 <- do.call(rbind, table_list)  # Using rbind()

by_8_2 %>%
  write_csv("yan/model2/table_by_8_2.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:6) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~  year + vmt + sourcehours + vehicles + starts +
                           year*vmt + year*sourcehours + vmt*sourcehours, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = fueltype_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_14_2 <- do.call(rbind, table_list)  # Using rbind()
by_14_2 %>%
  write_csv("yan/model2/table_by_14_2.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:5) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~  year + vmt + sourcehours + vehicles + starts +
                           year*vmt + year*sourcehours + vmt*sourcehours, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = roadtype_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_15_2 <- do.call(rbind, table_list)  # Using rbind()

by_15_2 %>%
  write_csv("yan/model2/table_by_15_2.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:9) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~  year + vmt + sourcehours + vehicles + starts +
                           year*vmt + year*sourcehours + vmt*sourcehours, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = regclass_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_12_2 <- do.call(rbind, table_list)  # Using rbind()

by_12_2 %>%
  write_csv("yan/model2/table_by_12_2.csv")
