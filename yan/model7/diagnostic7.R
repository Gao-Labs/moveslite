#' @title diagnostic workflow # 7
#' @author Yan
#' @describeIn diagnostic workflow with model 7
...
for (i in 1:3110) {
  ...
  table_list[[i]] <- diagnostic(.data = default, .formula = emissions ~ poly(vmt, 2) + vehicles + poly(sourcehours, 2) + poly(starts, 2) + year, .pollutant = .pollutant, .by = .by, .geoid = .table)

}

by_16_7 <- do.call(rbind, table_list)  # Using rbind()

by_16_7 %>%
  write_csv("yan/model7/table_by_16_7.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:13) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~ poly(vmt, 2) + vehicles + poly(sourcehours, 2) + poly(starts, 2) + year, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = sourcetype_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_8_7 <- do.call(rbind, table_list)  # Using rbind()

by_8_7 %>%
  write_csv("yan/model7/table_by_8_7.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:6) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~ poly(vmt, 2) + vehicles + poly(sourcehours, 2) + poly(starts, 2) + year, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = fueltype_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_14_7 <- do.call(rbind, table_list)  # Using rbind()
by_14_7 %>%
  write_csv("yan/model7/table_by_14_7.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:5) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~ poly(vmt, 2) + vehicles + poly(sourcehours, 2) + poly(starts, 2) + year, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = roadtype_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_15_7 <- do.call(rbind, table_list)  # Using rbind()

by_15_7 %>%
  write_csv("yan/model7/table_by_15_7.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:9) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~ poly(vmt, 2) + vehicles + poly(sourcehours, 2) + poly(starts, 2) + year, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = regclass_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_12_7 <- do.call(rbind, table_list)  # Using rbind()

by_12_7 %>%
  write_csv("yan/model7/table_by_12_7.csv")
