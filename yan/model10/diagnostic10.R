#' @title diagnostic workflow # 10
#' @author Yan
#' @describeIn diagnostic workflow with model 10


# model = lm(emissions ~ poly(vmt, 3) + year + vmt*year, data = default)
...
for (i in 1:3110) {
  ...
  table_list[[i]] <- diagnostic(.data = default, .formula = emissions ~ poly(vmt, 3) + year + vmt*year, .pollutant = .pollutant, .by = .by, .geoid = .table)

}

by_16_10 <- do.call(rbind, table_list)  # Using rbind()

by_16_10 %>%
  write_csv("yan/model10/table_by_16_10.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:13) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~ poly(vmt, 3) + year + vmt*year, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = sourcetype_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_8_10 <- do.call(rbind, table_list)  # Using rbind()

by_8_10 %>%
  write_csv("yan/model10/table_by_8_10.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:6) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~ poly(vmt, 3) + year + vmt*year, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = fueltype_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_14_10 <- do.call(rbind, table_list)  # Using rbind()
by_14_10 %>%
  write_csv("yan/model10/table_by_14_10.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:5) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~ poly(vmt, 3) + year + vmt*year, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = roadtype_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_15_10 <- do.call(rbind, table_list)  # Using rbind()

by_15_10 %>%
  write_csv("yan/model10/table_by_15_10.csv")
...
for (i in 1:3110) {
  ...
  for (j in 1:9) {
    ...
    result <- diagnostic(.data = default, .formula = emissions ~ poly(vmt, 3) + year + vmt*year, .pollutant = .pollutant, .by = .by, .geoid = .table, .type = regclass_1[j])
    table_list <- append(table_list, list(result))
  }

}

by_12_10 <- do.call(rbind, table_list)  # Using rbind()

by_12_10 %>%
  write_csv("yan/model10/table_by_12_10.csv")
